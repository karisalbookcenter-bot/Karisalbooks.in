...
// Namespace import, matching category.service.ts/subcategory.service.ts's
// own function-export shape (same convention as bookService above) —
...
// Sprint 16: categories/subcategories are now fetched internally, the same
// way authors/publishers already were — no more required props, no more
// passing [] from app/admin/books/page.tsx (Sprint 15's documented gap).
export function BookManagementOverview() {
  ...
  const loadBooks = useCallback(async () => {
    ...
    const result = await bookService.listBooks({
      ...
      // ListBooksInput.statuses is RecordStatus[] — send undefined when
      // none picked, not an empty array (an empty array could get treated
      // as an .in() filter matching nothing, not "no filter").
      statuses: filters.statuses.length > 0 ? filters.statuses : undefined,
      // ListBooksInput.sortBy is `keyof Book`, not `string` — BOOK_SORT_OPTIONS'
      // ids ("title", "price", "stock_quantity", "created_at") are all real
      // Book keys, so this cast is narrowing to something already true, not
      // bypassing a real mismatch.
      sortBy: filters.sortBy as keyof Book,
  ...
  // Authors/Publishers/Categories/Subcategories all have real backends now
  ...
  const handleBulkAction = async (actionId: string) => {
    if (actionId === "delete") {
      ...
      await bookService.bulkUpdateBooksStatus(selectedIds, status);
    }
    setSelectedIds([]);
    loadBooks();
  };
